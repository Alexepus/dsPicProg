...
void ProcessUart1Msg(void *par)
{
 ...
 while (1)
 {
  tn_sem_wait(&UartRcMsgSem, (TN_TickCnt)0xFFFFFFFF);
  ...
  case ((BYTE)0x0B):
  {
   ...
   else if(RcBuf.Data[1]==1)
   {
    FlagWait=0;
   }
   else if(RcBuf.Data[1]==2)
   {
    FlagMainOff=1;
    TRef=0;
  ...
  case ((BYTE)0xF0):
  {
   if(RcBuf.Length==3)
   {
    ...
    TxBuf.Data[0]=((BYTE)0xF0);
    TxBuf.Data[1]=*Address;
    TxBuf.Length=2;
    UartStartTx();
   }
   else
    {TxBuf.Data[0]=0xE1; TxBuf.Length=1; UartStartTx();};
   break;
  }
  case ((BYTE)0xF1):
  {
  ...
  case ((BYTE)0xFD):
  {
   if((RcBuf.Length==2) && (RcBuf.Data[1]==0x55))
            {
                __asm__("RESET");
            }
   else
...
void ReportStartUp(void)
{
 TxBuf.Data[0]=((BYTE)0xFF);
# 405 "ProcessUartMsg.c"
  TxBuf.Data[1]=7;
