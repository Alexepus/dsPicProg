...
void UartStartTx(void)
{
 ...
 do
 {
  ...
  if(WasTxShift)
  {
   TxFifo.Buffer[TxFifo.NIn] = DataByte-0xAA;
   ++i;
   WasTxShift=0;
  }
  else if((DataByte==0xAA) || (DataByte==0xAC) || (DataByte==0xAB))
  {
   TxFifo.Buffer[TxFifo.NIn] = 0xAC;
   WasTxShift=1;
  }
  else
  {
...
UINT ExtractRcMessage()
{
 ...
 while(1)
 {
  ...
  else if(DataByte==0xAA)
  {
   ReportRcProtocolError();
   WindRcFifo();
   return 0;
  }
  else if(DataByte==0xAB)
  {
   if(RcBuf.Length == 0)
   {
    ReportRcProtocolError();
  ...
  else
  {
   ...
   {
    RcBuf.Data[RcBuf.Length]=DataByte+0xAA;
    WasRxShift=0;
    ++RcBuf.Length;
   }
  }
